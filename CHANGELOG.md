...
- Review fixes rolled in: the currency is passed into the render path (a `with_state` call inside render would deadlock the game, since render already holds the state mutex), the euro sign is in the font ranges, failed/cancelled/reply-only Choya runs are recorded, a cancelled or panicked reopen fails the measuring step so the opening card can be dismissed, step explanations are stored as i18n keys instead of frozen text, on-disk steps are capped and the log rotates at 20 MB, a partial provider-reported cost reads "n/a" instead of an undercount, and filtered/sorted rows are cached until they change.
